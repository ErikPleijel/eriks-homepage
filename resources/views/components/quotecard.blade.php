...
@if ($text !== null)
    ...
    @php
        ...
        // Allow &nbsp; in header/text props to represent real non-breaking spaces
        // (the canvas draws raw text, not HTML, so the entity itself must be converted).
        $header = $header ? str_replace('&nbsp;', "\u{A0}", $header) : $header;
        $text   = $text   ? str_replace('&nbsp;', "\u{A0}", $text)   : $text;
    @endphp
    ...
    <script>
        (function () {
            ...
            function drawCardContent(ctx, W, H, preview) {
                ...
                const FOOTER_LINE_H   = 44;
                const FOOTER_TOP      = FOOTER_BOTTOM - FOOTER_LINE_H - 20;

                ctx.textBaseline = 'alphabetic';
                ctx.textAlign    = 'left';
                ctx.font         = 'italic 38px Georgia, serif';
                ctx.fillStyle    = 'rgba(255,211,122,0.95)';
                ctx.fillText(BOOK_TITLE, CONTENT_L, FOOTER_BOTTOM - FOOTER_LINE_H);
                ctx.font      = '42px Georgia, serif';
                ctx.fillStyle = 'rgba(255,255,255,0.90)';
                ctx.fillText(SITE_DOMAIN, CONTENT_L, FOOTER_BOTTOM);
